...
func CSTrainingsList(v CSTrainingsListView) g.Node {
	...
	if v.Err != "" {
		body = append(body, ui.Toast(ui.VariantDestructive, "cs-trainings-err", g.Text(v.Err)))
	}
	if v.Msg != "" {
		body = append(body, ui.Toast(ui.VariantSuccess, "cs-trainings-ok", g.Text(v.Msg)))
	}
	if len(v.Items) == 0 {
...
// csTrainingsTable & csTrainingTableRow (tabel daftar training) di
// cs_trainings_list_table.go. csTrainingStatusForm (aksi ganti status per
// baris) di cs_trainings_actions.go.
