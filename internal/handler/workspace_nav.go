...
func workspaceNav(slug string, canMembers, canSettings, canAccounts, canContacts, canLeads, canDeals, canSalesActivity, canPlans, canSubs, canSLA, canRoles bool) []ui.NavItem {
	items := []ui.NavItem{
	...
	// berbackend (enabled per izin, slice A1), sisanya placeholder. Selalu
	// tampil agar peta jalan terlihat.
	items = append(items, workspaceCSGroup(slug, canSLA))
