...
func workspaceNav(slug string, canMembers, canSettings, canAccounts, canContacts, canLeads, canDeals, canSalesActivity, canPlans, canSubs, canSLA, canPlaybooks, canRoles bool) []ui.NavItem {
	items := []ui.NavItem{
	...
	// (slice A1) & Playbooks (slice A2) berbackend (enabled per izin),
	// sisanya placeholder. Selalu tampil agar peta jalan terlihat.
	items = append(items, workspaceCSGroup(slug, canSLA, canPlaybooks))
