...
func workspaceNavCtx(ctx context.Context, slug string) []ui.NavItem {
	...
	return workspaceNav(slug, canViewMembers(ctx), canEditWorkspace(ctx), canViewAccounts(ctx), canViewContacts(ctx), canViewLeads(ctx), canViewDeals(ctx), canViewSalesActivity(ctx), canViewAllActivities(ctx), canViewPlans(ctx), canViewSubscriptions(ctx), canViewSLAPolicies(ctx), canViewPlaybooks(ctx), canViewKBArticles(ctx), canManageRoles(ctx), canViewTickets(ctx), canViewHealthScore(ctx), canViewSuccessPlans(ctx), canViewEngagements(ctx), canViewCSRenewals(ctx), canViewSalesReports(ctx), canViewCSReports(ctx), canViewSupportReports(ctx), canViewSubscriptionReports(ctx), canReadCSJourney(ctx), canViewTrainings(ctx))
}
