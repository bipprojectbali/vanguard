...
//
//   - DAFTAR GLOBAL: nomor HP/WhatsApp (mobile_phone) ikut di baris tapi TETAP
//     tunduk F4 (Sales utuh, non-Sales tersamar) — masking berlaku di daftar,
//     bukan hanya detail.
...
// --- Daftar global: masking HP/WhatsApp + kolom Desa ----------------------

// TestContacts_ListWhatsappMasking: di DAFTAR global, nomor HP/WhatsApp utuh
// untuk Sales & Admin (pengelola workspace); role yang masih ter-mask (Manager)
// menerima penanda tersamar — nomor asli tak pernah sampai ke browser mereka.
// Membuktikan F4 menjaga daftar, bukan cuma detail. Kolom kini bersumber dari
// mobile_phone (HP & WhatsApp digabung jadi satu field) → assert atas nomor HP.
func TestContacts_ListWhatsappMasking(t *testing.T) {
	env, uid := setupAccounts(t)
	mobile := "0812-0000-0000"
	a := env.seedAccount(t, "Desa WA", &uid, nil, nil)
	env.seedContactPhone(t, a.ID, "Sari", mobile, "0813-2222-3333", "021-555-0000")
	...
	for _, role := range []string{"sales", "admin"} {
		...
		if !strings.Contains(body, mobile) {
			t.Errorf("%s harus melihat nomor HP/WhatsApp utuh di daftar", role)
		}
	}
	...
	if strings.Contains(mgrBody, mobile) {
		t.Error("Manager BOCOR — nomor HP/WhatsApp asli sampai ke daftar")
	}
	if !strings.Contains(mgrBody, flsHidden) {
		t.Error("Manager harus menerima penanda tersamar (flsHidden) di kolom HP/WhatsApp")
	}
}
