...
func navFor(ctx context.Context) []ui.NavItem {
	...
	return workspaceNav(session.TenantSlug(ctx), canManageMembers(ctx), canEditWorkspace(ctx), canViewAccounts(ctx), canViewContacts(ctx), canViewLeads(ctx), canViewDeals(ctx), canViewSalesActivity(ctx), canManageRoles(ctx))
}
