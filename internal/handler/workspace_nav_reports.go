...
// jalan). Sales Reports (8.1), Customer Success Reports (8.2), Support Reports
// (8.3), Subscription Reports (8.4) berbackend, enabled mengikuti canReports
// (objek crm:reports read, SATU gerbang utk semua preset). Custom Reports
...
func workspaceReportsGroup(slug string, canReports bool) ui.NavItem {
	...
	cs := ui.NavItem{Label: "Customer Success Reports", Icon: lucide.HeartPulse(html.Class("size-4"))}
	if canReports {
		cs.Href = wsPath(slug, "/reports/customer-success")
	} else {
		cs.Disabled = true
	}
	support := ui.NavItem{Label: "Support Reports", Icon: lucide.Headset(html.Class("size-4"))}
	if canReports {
		support.Href = wsPath(slug, "/reports/support")
	} else {
	...
	children := []ui.NavItem{sales, cs, support, subs}
	return ui.NavItem{
