...
//
// Validasi & resolusi PRA-tx (guard tulis, parse form, resolusi desa & cek
// dup) di sales_convert_action_input.go — keduanya SEBELUM tx dimulai.
...
func (h *Handler) LeadConvert(w http.ResponseWriter, r *http.Request) {
	l, form, id, ok := h.resolveConvertRequest(w, r)
	if !ok {
		return
	}
	ctx := r.Context()
	...
	reg, ok := h.resolveConvertVillage(w, r, form.VillageID, tenantID, convertErr)
	if !ok {
		return
	}
	vcode := reg.Code
